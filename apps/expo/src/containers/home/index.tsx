...
export default function Home() {
  ...
  const isAfternoon =
    Number(moment().format("HH")) >= 12 && Number(moment().format("HH")) < 18;
  const isNight = Number(moment().format("HH")) >= 18;
  const authContext = useContextHook();
  const pendingInvoices = api.mk.getPendingInvoices.useQuery({
    session: authContext.session,
  });
  const lastInvoice = pendingInvoices.data?.FaturasPendentes?.filter(
    (invoice) =>
      invoice.contratos.includes(
        `Contrato: ${authContext.selectedConnection?.contract?.codcontrato}`,
      ),
  )?.[0];
  console.log(authContext.connections.data?.Conexoes);
  return (
        ...
        <View className="flex-row items-center justify-between">
          <View>
            ...
            <Text className="text-lg font-bold text-white">
              {authContext.clientInfo.data?.nome}
            </Text>
          </View>
          <LogoutButton />
        </View>
